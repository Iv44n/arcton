...
export interface ArctonListenOptions {
  ...
  /**
   * Drains in-flight requests on SIGTERM/SIGINT before exiting, process-wide.
   * On signal, it stops accepting new connections, waits (up to
   * `shutdownTimeout`), then calls `process.exit()` — ending the whole
   * process, not just this server. Set to `false` when the host application
   * manages process signals itself.
   *
   * @default true
   */
  gracefulShutdown?: boolean
  /**
   * Max time (ms) to wait for in-flight requests before forcing the close.
   * Ignored when `gracefulShutdown` is `false`.
   * @default 10_000
   */
  ...
  const app: ArctonApp<{}> = {
    ...
    listen(options = {}) {
      ...
      if (
        ...
        throw new Error(
          'shutdownTimeout must be a positive, finite number of milliseconds.'
        )
      }
      ...
      const untrack =
        options.gracefulShutdown === false
          ? undefined
          : shutdownCoordinator.track(
              server,
              options.shutdownTimeout ?? DEFAULT_SHUTDOWN_TIMEOUT_MS
            )
